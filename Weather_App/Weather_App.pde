...
void setup() {
  ...
  textDraw(nameTor, titleFont, height, #2C08FF, LEFT, CENTER, apiCallX3, apiCallY3, apiCallRectWidth3, apiCallRectHeight3); //Toronto
}
...
void draw () {
  ...
  if (mouseX>quitX && mouseX<quitX+quitRectWidth && mouseY>quitY && mouseY<quitY+quitRectHeight){
    ...
    textDraw(quit, titleFont, height, #2C08FF, CENTER, CENTER, quitX, quitY, quitRectWidth, quitRectHeight);
} else {
  fill(white);
  rect(quitX, quitY, quitRectWidth, quitRectHeight);
  textDraw(quit, titleFont, height, #2C08FF, CENTER, CENTER, quitX, quitY, quitRectWidth, quitRectHeight);
...
void mousePressed() {
  ...
  if ( mouseX>apiCallX1 && mouseX<apiCallX1+apiCallRectWidth1 && mouseY>apiCallY1 && mouseY<apiCallY1+apiCallRectHeight1 ) {
    ...
    rect(currentTempX, currentTempY, currentTempRectWidth, currentTempRectHeight);
    textDraw(torTemp, titleFont, height, #2C08FF, CENTER, CENTER, currentTempX, currentTempY, currentTempRectWidth, currentTempRectHeight); //Toronto
